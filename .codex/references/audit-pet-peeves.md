...
**Example:** PR #87. `.codex/hooks/log-reminder.py` docstring said "blocks Claude from stopping" but the code had been rewritten to stderr-only advisories. Copilot caught.
...
**How to catch.** Not mechanical — it's a judgment pattern. The rule `.codex/rules/summary-parity.md` encodes it: two review-bot flags on the same summary paragraph = rewrite structurally (abstract up), don't patch.
...
**Example:** PR #93 (Copilot). `check-skill-integrity.py` had `RULE_KEYWORDS` entries for `cross-artifact-review.md` and `content-invariants.md`, but neither rule's scope frontmatter (one uses `globs:`, both target `.tex`/`.qmd` files not `.codex/skills/*`) actually fires the check. The entries were no-ops. A future maintainer reading the code would reasonably assume the check was exercising those rules.
...
A future mechanical check could count `.codex/{skills,agents,rules}/*` and grep table rows in these surfaces; until then, deep-audit Agent 1 + Agent 4 should explicitly check appendix table row counts.
...
**When to apply.** Whenever a release adds or removes a skill/agent/rule/hook. Surface-sync clean ≠ enumerative tables current. Run a quick `wc -l` style check of appendix tables vs `ls .codex/skills/ | wc -l`.
...
- Reference this file from `.codex/skills/deep-audit/SKILL.md` so all 4 agents load it.
