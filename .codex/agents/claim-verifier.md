...
## Role Activation (Codex CLI)

When the orchestrating model needs to invoke this agent role:
1. Read this file in full to adopt the persona
2. Maintain this role for the designated phase
3. Produce the specified output to the specified path
4. Return to your normal orchestrating role when done

For the **claim-verifier** only: always spawn via subprocess (`./scripts/run-workflow.sh --fork claim-verifier`) to ensure genuine context isolation. Never inline claim-verifier into the main session.
